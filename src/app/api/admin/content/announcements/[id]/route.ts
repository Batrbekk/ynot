...
import { AnnouncementUpdateSchema } from '@/lib/schemas/admin-announcement';
import {
  updateAnnouncement,
  deleteAnnouncement,
} from '@/server/admin/cms/announcement-service';
...
export async function PATCH(req: Request, ctx: Ctx): Promise<Response> {
  ...
  try {
    const a = await updateAnnouncement({
      id,
      input: parsed.data,
      actorId,
...
export async function DELETE(req: Request, ctx: Ctx): Promise<Response> {
  ...
  const { id } = await ctx.params;
  try {
    const a = await deleteAnnouncement({
      id,
      actorId,
      ip: req.headers.get('x-forwarded-for') ?? undefined,
      ua: req.headers.get('user-agent') ?? undefined,
