...
**Date:** 2026-04-30
**Status:** Approved (user review complete)
...
The migration also adds the **`order_number_seq`** Postgres sequence which the Phase 1 spec (§241) defined but the Phase 1 init migration omitted. Format: `YN-YYYY-NNNNN` (e.g. `YN-2026-00001`), generated by `nextOrderNumber(tx)` helper:
...
export async function nextOrderNumber(tx: Prisma.TransactionClient): Promise<string> {
  ...
  return `YN-${year}-${nextval.toString().padStart(5, '0')}`;
}
```

The sequence is monotonic across years (`YN-2026-00099` → `YN-2027-00100`); the year prefix is informational, not a reset boundary. This matches Phase 1's intent and avoids the rollover-collision foot-gun of per-year sequences.
...
return prisma.$transaction(async (tx) => {
  ...
  const order = await tx.order.create({
    data: {
      orderNumber: await nextOrderNumber(tx),  // YN-2026-00001 (format from Phase 1 §241; Postgres sequence)
      userId,
